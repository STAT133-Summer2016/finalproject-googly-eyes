...
cad_us_rate = read_csv("../raw_data/exchange rate/rates_data_cad.csv")
colnames(cad_us_rate) = c("date", "rate", as.character(1:9))
cad_us_rate = cad_us_rate %>% 
  mutate(date = ymd(date)) %>% 
  select(date, rate)

aud_us_rate = read_csv("../raw_data/exchange rate/rates_data_aud.csv")
...
for (year in years){
  df = read_csv(str_c("../raw_data/imdb/movies_imdb_", year, ".csv", sep = ""))
...
write_csv(actors_and_movies, "actors_and_movies.csv")
...
movies_rt = read_csv("../raw_data/movies_original_rt.csv")
...
write_csv(movies_rt, "movies_rt.csv")
View(movies_rt)
