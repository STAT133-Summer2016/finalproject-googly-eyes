...
for (suffix in movies$html_suffix){
  ...
  if (length(directed_by) < 2){
    ...
    directed_by = directed_by %>% 
      ...
      str_replace_all("^,", "") %>% 
      str_replace_all(",$", "")
      directed_by = str_c(directed_by, collapse = ",")
      directed_by = ifelse(length(directed_by)==0, NA, directed_by)
      written_by = page %>% 
      xml_find_all("//div[@class='col-sm-19 col-xs-14 text-left']") %>% 
      .[2] %>% 
      xml_text() %>% 
      ...
      written_by = str_c(written_by, collapse = ",")
      written_by = ifelse(length(written_by)==0, NA, written_by)
  }
  
  in_theaters = page %>% 
    xml_find_all("//div[@class='col-sm-19 col-xs-14 text-left']/time")
  
  if (length(in_theaters)==0){
    in_theaters = NA
  }else{
...
movies_df = bind_cols(data.frame(movies$Title), movies_df)
View(movies_df)
